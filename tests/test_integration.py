...
class TestCatalogReal:
    ...
    def test_list_projects_real(self, real_tools: Tools) -> None:
        ...
        assert "error" not in payload, (
            f"list_projects should succeed, got error: {payload.get('error')}"
        )
        data = _data(payload)
    ...
    def test_get_project_real(self, real_tools: Tools, real_config: Any) -> None:
        ...
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"get_project should succeed, got error: {payload.get('error')}"
        )
        data = _data(payload)
    ...
    def test_list_schemas_real(self, real_tools: Tools, real_config: Any) -> None:
        ...
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"list_schemas should succeed, got error: {payload.get('error')}"
        )
        data = _data(payload)
    ...
    def test_list_tables_real(self, real_tools: Tools, real_config: Any) -> None:
        project = real_config.default_project or "p1"
        r = real_tools.call(
            "list_tables", {"project": project, "schema": "default", "pageSize": 50}
        )
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"list_tables should succeed, got error: {payload.get('error')}"
        )
        data = _data(payload)
    ...
    def test_get_table_schema_real(self, real_tools: Tools, real_config: Any) -> None:
        project = real_config.default_project or "p1"
        list_r = real_tools.call(
            "list_tables", {"project": project, "schema": "default", "pageSize": 5}
        )
        ...
        table_name = (
            tables[0].get("tableName") or tables[0].get("name", "").split("/")[-1]
        )
        r = real_tools.call(
            "get_table_schema",
            {"project": project, "schema": "default", "table": table_name},
        )
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"get_table_schema should succeed, got error: {payload.get('error')}"
        )
        data = _data(payload)
    ...
    def test_get_partition_info_real(self, real_tools: Tools, real_config: Any) -> None:
        project = real_config.default_project or "p1"
        list_r = real_tools.call(
            "list_tables", {"project": project, "schema": "default", "pageSize": 20}
        )
        list_payload = _text_payload(list_r)
        assert "error" not in list_payload
        list_data = _data(list_payload)
        ...
        for t in tables:
            ...
            assert "partitions" in payload, (
                f"get_partition_info should return partitions: {payload}"
            )
            assert isinstance(payload["partitions"], list)
            return  # got valid response
        pytest.skip(
            "no partitioned table found in first page; get_partition_info requires a partitioned table"
        )
...
class TestSearchReal:
    ...
    def test_search_meta_data_real(self, real_tools: Tools, real_config: Any) -> None:
        ...
        r = real_tools.call(
            "search_meta_data", {"query": "type=table,name:test", "pageSize": 5}
        )
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"search_meta_data should succeed, got error: {payload.get('error')}"
        )
        assert "entries" in payload, (
            f"search_meta_data should return entries: {payload}"
        )
...
class TestComputeReal:
    ...
    def test_cost_sql_real(self, real_tools: Tools, real_config: Any) -> None:
        ...
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"cost_sql should succeed, got error: {payload.get('error')}"
        )
    ...
    def test_execute_sql_real(self, real_tools: Tools, real_config: Any) -> None:
        ...
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"execute_sql should succeed, got error: {payload.get('error')}"
        )
