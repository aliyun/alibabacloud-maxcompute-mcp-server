...
class TestCreateInsertTableTypes:
    ...
    def test_partitioned_table(
        ...
        assert (
            _count_rows(
                real_tools, project, table, hints={"odps.sql.allow.fullscan": "true"}
            )
            == 3
        )
    ...
    def test_if_not_exists_idempotent(
        self, real_tools: Tools, real_config: Any, created_tables: list[str]
    ) -> None:
        project = real_config.default_project
        ...
        assert p2.get("success") is True, (
            f"second create_table (ifNotExists) failed: {p2}"
        )
        assert _count_rows(real_tools, project, table) == 0

    def test_storage_tier(
        self, real_tools: Tools, real_config: Any, created_tables: list[str]
    ) -> None:
        project = real_config.default_project
    ...
    def test_transactional_table(
        ...
        if p.get("success") is not True:
            pytest.skip(
                f"transactional table not supported in this env: {p.get('error')}"
            )
    ...
    def test_typed_partition_column(
        ...
        r = real_tools.call(
            "create_table",
            {
                ...
                "partitionColumns": [
                    {"name": "hr", "type": "BIGINT", "description": "hour bucket"}
                ],
                "lifecycle": 1,
            },
        )
        ...
        assert p.get("success") is True, (
            f"insert into typed-partition table failed: {p}"
        )
        assert p.get("partitionBatches") == 2
        assert (
            _count_rows(
    ...
    def test_auto_partition_generate_expression(
        ...
        if p.get("success") is not True:
            pytest.skip(
                f"AUTO PARTITIONED BY not supported in this env: {p.get('error')}"
            )
        ...
        assert (
            _count_rows(real_tools, project, table, partition="sale_month='2026-01'")
            == 2
        )
        assert (
            _count_rows(real_tools, project, table, partition="sale_month='2026-02'")
            == 1
        )

    def test_table_properties_explicit(
        self, real_tools: Tools, real_config: Any, created_tables: list[str]
    ) -> None:
