...
def _assert_failure(payload: dict, context: str = "") -> None:
    ...
    assert has_error, (
        f"Expected failure{f' ({context})' if context else ''}, got: {payload}"
    )
...
class TestCatalogBrowseErrors:
    ...
    def test_get_schema_nonexistent(self, real_tools: Tools, real_config: Any) -> None:
...
class TestCreateTableErrors:
    ...
    def test_create_table_if_not_exists_is_idempotent(
        ...
        assert p1.get("success") is True, (
            f"First create_table (ifNotExists) failed: {p1}"
        )
