...
class TestGetProject:
    ...
    def test_get_project_known_project(
    ...
    def test_get_project_nonexistent_returns_error(self, real_tools: Tools) -> None:
        r = _call_safe(
            real_tools, "get_project", {"project": "nonexistent_project_xyz_12345"}
        )
        payload = _text_payload(r)
        # Either success=false with error, or an error key
        has_error = payload.get("success") is False or "error" in payload
        assert has_error, f"Expected error for non-existent project, got: {payload}"
...
class TestSchemas:
    ...
    def test_list_schemas_2tier_project_returns_default(
        ...
        schema_names = [
            s.get("schemaName") or s.get("name", "").split("/")[-1] for s in schemas
        ]
    ...
    def test_get_schema_nonexistent_returns_error(
        ...
        r = _call_safe(
            real_tools,
            "get_schema",
            {"project": project, "schema": "nonexistent_schema_xyz"},
        )
        payload = _text_payload(r)
        ...
        if payload.get("success") is True:
            ...
            assert (
                "schema" in desc.lower() or "2-level" in desc.lower() or d.get("name")
            ), (
...
class TestTables:
    ...
    def test_list_tables_basic(self, real_tools: Tools, real_config: Any) -> None:
        ...
        if not project:
            pytest.skip("default_project not configured")
        r = real_tools.call(
            "list_tables", {"project": project, "schema": "default", "pageSize": 10}
        )
        payload = _text_payload(r)
    ...
    def test_list_tables_filter_by_prefix(
        ...
        if payload.get("success") is False or "error" in payload:
            ...
            r2 = real_tools.call(
                "list_tables",
                {"project": project, "schema": "default", "pageSize": 100},
            )
            p2 = _text_payload(r2)
            ...
            table_names = [
                t.get("tableName") or t.get("name", "").split("/")[-1] for t in tables
            ]
    ...
    def test_get_table_schema_standard_table(
        ...
        if "columns" in d:
            assert isinstance(d["columns"], list), (
                f"columns must be a list: {type(d['columns'])}"
            )
    ...
    def test_get_table_schema_partitioned_table(
        ...
        logger.info(
            "Partitioned table schema keys: %s, partitionKeys: %s",
            list(d.keys()),
            partition_keys,
        )
    ...
    def test_get_table_schema_nonexistent(
        ...
        payload = _text_payload(r)
        has_error = payload.get("success") is False or "error" in payload
        assert has_error, f"Expected error for non-existent table, got: {payload}"
...
class TestPartitionInfo:
    ...
    def test_get_partition_info_partitioned_table(
        ...
        assert "error" not in payload, (
            f"get_partition_info error: {payload.get('error')}"
        )
    ...
    def test_get_partition_info_unpartitioned_table(
        ...
        is_empty = (
            isinstance(payload.get("partitions"), list)
            and len(payload["partitions"]) == 0
        )
