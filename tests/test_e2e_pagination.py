...
def _paginate_list_projects(real_tools: Tools, *, page_size: int = 1) -> list[dict]:
    ...
    while pages < _MAX_PAGES:
        ...
        assert "error" not in payload, (
            f"list_projects page {pages} error: {payload.get('error')}"
        )
        d = _data(payload)
...
def _paginate_list_tables(
    ...
    *,
    ...
    while pages < _MAX_PAGES:
        ...
        payload = _text_payload(r)
        assert "error" not in payload, (
            f"list_tables page {pages} error: {payload.get('error')}"
        )
        d = _data(payload)
...
class TestListProjectsPagination:
    ...
    def test_full_pagination_no_duplicates(self, real_tools: Tools) -> None:
        ...
        names = [
            p.get("projectId") or p.get("name", "").split("/")[-1] for p in all_projects
        ]
    ...
    def test_full_pagination_matches_bulk_list(self, real_tools: Tools) -> None:
        ...
        paged_names = {
            p.get("projectId") or p.get("name", "").split("/")[-1] for p in paged
        }
        bulk_names = {
            p.get("projectId") or p.get("name", "").split("/")[-1] for p in bulk
        }
...
class TestListTablesPagination:
    ...
    def test_list_tables_pagination_covers_created_tables(
        ...
        retrieved_names = [
            t.get("tableName") or t.get("name", "").split("/")[-1] for t in all_tables
        ]
...
class TestGetPartitionInfoPagination:
    ...
    def test_partition_pagination_covers_all_partitions(
        ...
        all_parts = _paginate_get_partition_info(
            real_tools, project, table, page_size=1
        )
        if not all_parts:
            pytest.skip(
                "get_partition_info returned no partitions (may not be supported)"
            )
...
class TestInvalidTokenHandling:
    ...
    def test_list_projects_invalid_token(self, real_tools: Tools) -> None:
        ...
        assert isinstance(payload, dict), (
            f"Expected dict response, got: {type(payload)}"
        )
        ...
        if not has_error:
            ...
            assert isinstance(projects, list), (
                f"Expected projects list or error, got: {payload}"
            )
    ...
    def test_list_tables_invalid_token(
        ...
        if not has_error:
            ...
            assert isinstance(tables, list), (
                f"Expected tables list or error, got: {payload}"
            )
    ...
    def test_get_partition_info_invalid_token(
        ...
        if not has_error:
            ...
            assert isinstance(partitions, list), (
                f"Expected partitions list or error, got: {payload}"
            )
