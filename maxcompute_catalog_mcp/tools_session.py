...
if TYPE_CHECKING:
    from .config import MaxComputeCatalogConfig
...
class SessionMixin:
    ...
    if TYPE_CHECKING:
        _configs: dict[str, MaxComputeCatalogConfig]
        _current_name: str
        _default_name: str
    ...
    def list_configs(self, args: dict[str, Any]) -> dict[str, Any]:
        configs = [self._config_view(n) for n in self._configs]
    ...
    def get_current_config(self, args: dict[str, Any]) -> dict[str, Any]:
        if self._current_name not in self._configs:
            return mcp_text_result(
                {"success": False, "error": "no current config is set"}
            )
        return mcp_text_result(
            {"success": True, "data": self._config_view(self._current_name)}
        )
    ...
    def use_config(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("Failed to switch to config %r", name)
            return mcp_text_result(
                {
                    "success": False,
                    "error": (
