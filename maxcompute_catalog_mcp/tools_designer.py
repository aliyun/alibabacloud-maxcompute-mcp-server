...
def _get_max_partition_batches() -> int:
    ...
    try:
        ...
        logger.warning("Invalid MAX_PARTITION_BATCHES=%r, using default 100", val)
        return 100
...
class DesignerMixin:
    ...
    def create_table(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if table_properties is not None and not isinstance(table_properties, dict):
            return mcp_text_result(
                {
                    "success": False,
                    "error": "tableProperties must be an object of string key/value pairs.",
                }
            )
        if hints is not None and not isinstance(hints, dict):
            return mcp_text_result(
                {
                    "success": False,
                    "error": "hints must be an object of string key/value pairs.",
                }
            )
        if primary_key is not None and not isinstance(primary_key, list):
            return mcp_text_result(
                {
                    "success": False,
                    "error": "primaryKey must be an array of column names.",
                }
            )
        ...
        if not columns:
            return mcp_text_result(
                {"success": False, "error": "columns must not be empty."}
            )
        try:
            ...
            table_name = (
                f"{schema}.{table}" if schema and schema != "default" else table
            )
            ...
            part_objs = [
                _build_column(pc, is_partition=True) for pc in partition_columns
            ]
    ...
    def insert_values(self, args: dict[str, Any]) -> dict[str, Any]:
        if not self.maxcompute_client:
            return _unsupported(
                "INSERT INTO ... VALUES requires MaxCompute compute engine (default_project + sdk_endpoint)."
            )
        project = opt_arg(args, "project", self.default_project) or self.default_project
        schema = opt_arg(args, "schema", "default") or "default"
        table = require_arg(args, "table", "Table name cannot be empty")
        ...
        if not columns or not values:
            return mcp_text_result(
                {"success": False, "error": "columns and values cannot be empty"}
            )
        try:
            ...
            def _col_name(c: Any) -> str:
                return (
                    c if isinstance(c, str) else c.get("name", c.get("columnName", ""))
                ) or ""

            column_names = [_col_name(c) for c in columns]
            if len(column_names) != len(columns) or any(not n for n in column_names):
                return mcp_text_result(
                    {"success": False, "error": "Empty column name found in columns"}
                )
            ...
            for row in values:
                row_vals = [
                    _quote_sql_value(row[i]) if i < len(row) else "NULL"
    ...
    def _insert_values_partitioned(
        ...
        for pc in partition_cols:
            if pc not in column_names:
                return mcp_text_result(
                    {
                        "success": False,
                        "error": f"partitionColumns '{pc}' not found in columns",
                    }
                )
        ...
        if not data_col_names:
            return mcp_text_result(
                {
                    "success": False,
                    "error": "At least one data column required besides partition columns",
                }
            )
        ...
        for row in values:
            if len(row) < len(column_names):
                return mcp_text_result(
                    {
                        "success": False,
                        "error": f"Row length {len(row)} does not match columns count {len(column_names)}",
                    }
                )
        ...
        for pk, rows in groups.items():
            ...
            if async_mode:
                ...
                except Exception as e:  # noqa: BLE001 -- preserve partial batch results.
            ...
            if timed_out:
                return _build_timeout_response(
                    inst, project, timeout_secs, "Insert", " on partition batch"
                )
            total_inserted += len(rows)
